    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "train" or self.mode == "val":
            ...
            if self.visu:
                ...
                return (motion_vectors_ret, boxes_prev_ret, velocities_ret,
                    num_boxes_mask_ret, boxes_ret, gt_ids_ret, gt_ids_prev_ret,
                    frame_ret)
...
if __name__ == "__main__":
    batch_size = 2
    window_lenght = 1
    datasets = {x: MotionVectorDataset(root_dir='data', window_length=window_lenght, codec="h264", visu=True, mode=x) for x in ["train", "val", "test"]}
    ...
    cv2.namedWindow("mvs", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("mvs", 640, 360)

    for step, (motion_vectors_, boxes_prev_, velocities_, num_boxes_mask_,
        boxes_, gt_ids_, gt_ids_prev_, frames_) in enumerate(dataloaders["train"]):
        ...
        for batch_idx in range(batch_size):
            ...
            if window_lenght == 1:
                ...
                mvs_image = draw_boxes_on_motion_vector_image(mvs_image, boxes)
                cv2.imshow("mvs", mvs_image)
