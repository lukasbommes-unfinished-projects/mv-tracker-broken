...
if __name__ == "__main__":
    batch_size = 2
    window_lenght = 1
    datasets = {x: MotionVectorDataset(root_dir='data', window_length=window_lenght, codec="mpeg4", mode=x) for x in ["train", "val", "test"]}
    ...
    for step, (motion_vectors_, boxes_prev_, velocities_, num_boxes_mask_, boxes_,
        ...
        for batch_idx in range(batch_size):
            ...
            if window_lenght == 1:
                ...
                #boxes = boxes[num_boxes_mask[batch_idx], :]
                #boxes_prev = boxes_prev[num_boxes_mask[batch_idx], :]
                #velocities = velocities[num_boxes_mask[batch_idx], :]
